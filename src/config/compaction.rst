...
.. config:section:: compactions :: Compaction Daemon Rules
    ...
    - ``view_fragmentation``: If the ratio of legacy data, including metadata,
      ...
      percentage is expressed as an integer percentage. This value is
      computed as::
    ...
    to compact a particular database or view index, a warning message is logged.
    ...
    #. ``[{db_fragmentation, "70%"}, {view_fragmentation, "60%"}, {from, "00:00"}, {to, "04:00"}]``
       ...
       index) is only triggered if the current time is between midnight and
       4 AM.
