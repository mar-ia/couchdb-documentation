...
.. config:section:: admins :: Server Administrators
    ...
    .. versionchanged::
        1.4 `PBKDF2` server-side hashed salted password support
...
.. config:section:: couch_httpd_auth :: Authentication Configuration
    ...
    .. config:option:: iterations :: PBKDF2 iterations count
        ...
        algorithm. A higher  number provides better hash durability, but
        comes at a cost in performance for each request that requires
        authentication.
    ...
    .. config:option:: public_fields :: User documents public fields
        ...
        :option:`couch_httpd_auth/authentication_db`) that can be read by
        any user. If unset or not specified, authenticated users can only
